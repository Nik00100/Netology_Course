...
public class Server {
    final static String PATH_TO_PROPERTIES = "Server/src/main/resources/application.properties";
    ...
    public static void main(String[] args) throws IOException {
        int port = -1;
        try {
            serverProperties = new ServerProperties(PATH_TO_PROPERTIES);
            port = serverProperties.getPort();
        } catch (IOException e) {
            ConsoleHelper.writeMessage("Произошла ошибка при чтении файла настроек параметров сервера");
        }
    ...
    public static void sendBroadcastMessage(Message message) {
        for (Connection connection : connectionMap.values()) {
            try {
                ...
                logger.log("Сервером отправлено сообщение всем участникам чата " + message.getData());
            } catch (IOException e) {
